...
//diag start
p1 = polarToXY([diagonal_start_depth,mid-off]);
//diag cut
p2 = polarToXY([diagonal_depth_single,mid-off]);
//notch cut
p3 = polarToXY([r,ang-off]);
// notch start
p4 = polarToXY([notch_start_depth,ang-off]);
mp_ang = (convexity_weight)*ang+(1-convexity_weight)*mid;
// flare
pf = (ang%90<45 && ang%90>0) ||(ang%90<0&&ang%90<-45)?
    [p3[0]-flare_length*cos(ang+flare_ang-off),p3[1]-flare_length*sin(ang+flare_ang-off)]
    :[p3[0]-flare_length*cos(ang-flare_ang-off),p3[1]-flare_length*sin(ang-flare_ang-off)];
//echo(atan2(p3[1],p3[0]));
//echo(atan2(p3[1]-pf[1],p3[0]-pf[0]));
mp_r = (1-convexity)*r;
bp =  polarToXY([mp_r,mp_ang-off]);   
//nmp=(p2+p3)/2;
//bp = nmp*(1-convexity);
//make curve
...
p1 = polarToXY([diagonal_start_depth,mid-off]);
//notch1 start
p2 = polarToXY([notch_start_depth,a1-off]);
//notch1 cut 
p3 = polarToXY([r1,a1-off]);
//diag cut
p4 = polarToXY([diagonal_depth_double,mid-off]);
//notch2 cut 
p5 = polarToXY([r2,a2-off]);
//notch2 start    
p6 = polarToXY([notch_start_depth,a2-off]);
//flare notch1
pf1=[p3[0]-flare_length*cos(a1+flare_ang-off),p3[1]-flare_length*sin(a1+flare_ang-off)];
// flare notch2
pf2=[p5[0]-flare_length*cos(a2-flare_ang-off),p5[1]-flare_length*sin(a2-flare_ang-off)];
...
bp1 =  polarToXY([mp_r1,mp_a1-off]);   
bp2 =  polarToXY([mp_r2,mp_a2-off]);   
//nmp1=(p3+p4)/2;
